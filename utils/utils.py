...
def compute_loss(p, targets, model):  # predictions, targets, model
    # pred: 2x3x20x20x85,  2x3x40x40x85, 2x3x80x80x85  targets: 31x6
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor
    lcls, lbox, lobj = ft([0]), ft([0]), ft([0])
    # tbox' shape:[93x4, 135x4, 135x4]   tcls's shape:[93, 135, 135]
    # indices: a list of 3 items which contain 4 tuples whose shape are [93 135 135]
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        tobj = torch.zeros_like(pi[..., 0])  # target obj  2x3x20x20

        nb = b.shape[0]  # number of targets
        if nb:
            nt += nb  # cumulative targets
            ps = pi[b, a, gj, gi]  # prediction subset corresponding to targets  shape: 93x85
            ...
            # Class
            # yr: ps[..., 4] predict forground or background, it is not necessary to calculate class loss
            # if only 1 class, because lobj represent it
...
def build_targets(p, targets, model):
    ...
    # anchor tensor: num_anchor x num_targets 3x31, one row corresponding to id of anchors whose size is num_targets
    at = torch.arange(na).view(na, 1).repeat(1, nt)  # anchor tensor, same as .repeat_interleave(nt)

    style = 'rect4'
    for i in range(det.nl):  # yr: what is nl ?  number of yolo layer
        anchors = det.anchors[i]
        # gain[2:] represents w,h,w,h of current feature map of current yolo layer
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain

        # Match targets to anchors
        # t: num_targets x 6 = 31x6, every row represents [image, class, x, y, w, h] where x,y,w,h are
        # absolute anchor coordinates and width and height on current scale of current yolo feature map
        ...
        if nt:
            ...
            '''
            temp1 = torch.max(r, 1. / r)  # shape: 3x1x2
            temp2 = torch.max(r, 1. / r).max(2)   # return tuple of value and index
            temp3 = torch.max(r, 1. / r).max(2)[0]  # max w,h ratio of three anchors 
            # yr, j: 3x31, represents which anchor is assigned to target, in pi yolo layer, p1 find five   
            # matching gt boxes anchors
            '''
            # temp1 = torch.max(r, 1. / r)  # shape: 3x31x2
            # temp2 = torch.max(r, 1. / r).max(2)   # return tuple of value and index
            # temp3 = torch.max(r, 1. / r).max(2)[0]  # max value of w,h ratio of three anchors
            # j:3x31
            j = torch.max(r, 1. / r).max(2)[0] < model.hyp['anchor_t']  # compare  ???
            # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n) = wh_iou(anchors(3,2), gwh(n,2))

            # a: shape is 31, one item represent anchor ids match chosen gt boxes,
            # t: 31x6, one row represents one gt box labels [image, class, x, y, w, h]
            a, t = at[j], t.repeat(na, 1, 1)[j]  # filter

            # overlaps
            gxy = t[:, 2:4]  # grid xy   yr, gxy:31x2  represent ground truth of xy
        ...
        gxy = t[:, 2:4]  # grid xy  93x2
        gwh = t[:, 4:6]  # grid wh  93x2
    ...
    # tcls: 3 items list, one item is a tensor which contains classes of matching gt boxes
    # tbox: [93x4, 36x4, 42x4]  ### 15, 36, 42 represent anchor number????
    # indices: 3 items list, one item is a tuple whice contains image id, anchors id, x and y coordinate
    # of grid of gt boxes
